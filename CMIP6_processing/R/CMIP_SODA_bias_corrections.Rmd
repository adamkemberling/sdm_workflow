...
# Rendering Output:
# rmarkdown::render("CMIP6_processing/R/CMIP_SODA_bias_corrections.Rmd", params = list(variable = "surf_sal", climate_start = "1985", climate_stop = "2014"))
...
###  Load Collection of CMIP Models
...
### Check Date Format Using Xarray

Sometimes the raster package has an issue with the dates used by the climate models, more specifically how its a non-standard calendar without leap years. 

Xarray doesn't really have an issue for some reason, so this step matches the file to what the dates should be, to ensure proper date comparisons down the line.
...
```{r}
plot(cmip_data[[1]][[1]],
     main = str_c("Single Month of CMIP6 Data - ", pick_var),
     col = temp_pal)
```
...
# boiling water seems like a reasonable limit for temperature
...
The function that converts cmip data to climate anomalies relies on a consistent naming structure for each time step of XYYYYMMDD to pull out the correct years and months. 


If this isn't the case the function as currently written will break. To make sure this doesn't happen the dates and some additional metadata were put into a lookup table.
...
## Build Individual CMIP Climatologies

For each of the CMIP6 models there will be a climatology that tracks average the intra-annual variation across the variable of interest. 
...
var_path <- paste0(cmip_path, "BiasCorrected/EnsembleData/", pick_var, "/", pick_var)
