...
There are two data sources with which the CMIP6 model outputs are checked against to perform the bias-corrections. These are **1. OISSTv2** and **2. SODA reanalysis** data. Monthly-averaged timeseries for each region can be viewed below.
...
deg_c <- "\u00b0C"
region <- "EPU_GOM"
plot_vars <- c("Surface Temperature", "Bottom Temperature")

gom_ssp <- filter(ssps_annual, Region == region, variable %in% plot_vars) %>% 
      mutate(Region = "Gulf of Maine")
gom_obs <- filter(reference_annual, Region == region, variable %in% plot_vars) %>% 
      mutate(Region = "Gulf of Maine")

ggplot() +
  # Confidence Intervals
  geom_ribbon(
    data = filter(gom_ssp, scenario == "SSP1: 2.6"),
    aes(x = year, ymin = `5th Percentile`, ymax = `95th Percentile`, fill = scenario),
  ...
  scale_fill_gmri() +
  scale_color_manual(values = c("black", "black", gmri_cols("blue"), gmri_cols("orange"))) +
  scale_x_continuous(expand = expansion(add = c(3,0))) +
  scale_y_continuous(labels = scales::label_number(suffix = deg_c)) +
  facet_grid(fct_rev(variable)~Region, scales = "free") +
  theme_gmri() +
  guides(color = guide_legend(order = 1, nrow = 2, title.position = "top", title.hjust = 0.5), 
         fill = guide_legend(order = 2, title.position = "top", title.hjust = 0.5)) +
  labs(fill = "Climate Ensemble Uncertainty\n(5th-95th Percentile Range)", 
       color = "Average Annual Temperature:", 
       x = "", 
       y = str_c("Temperature"),
       title = "Gulf of Maine Observed and Projected Climates") +
