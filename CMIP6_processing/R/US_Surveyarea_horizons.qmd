...
Baseline conditions are referenced against 2010-2019 for both environmental conditions and baseline fish species availability.
...
# Monthly averages have been prepared in: 
# CMIP6_processing/R/helper01_regional_timeseries_construction.R
...
modern_comparison_annual <- ssp_both %>%
  # filter(year>= 2000) %>%
  pivot_longer(
    cols = c(`5th Percentile`, `Ensemble Mean`, `95th Percentile`), 
    names_to = "ensemble_statistic", values_to = "ssp_val") %>% 
...
# Get the distance from the baseline mean for monthly data
modern_comparison_monthly <- ssp_both %>%
  # filter(year>= 2000) %>%
  pivot_longer(
    cols = c(`5th Percentile`, `Ensemble Mean`, `95th Percentile`), 
    names_to = "ensemble_statistic", 
    values_to = "ssp_val") %>% 
  mutate(var = case_when(
    variable == "Surface Temperature" ~ "surf_temp",
    variable == "Bottom Temperature" ~ "bot_temp",
...
### Exporting Projection Timeseries
...
# projection_res_path <- paste0(project_box_path, "data/bias_corrected_env_projections/")
# 
# # Baseline Period Data
# write_csv(references_combined, str_c(projection_res_path, "oisst_soda_regional_timeseries.csv"))
# 
# # SSP scenario for regions
# write_csv(ssp_both, str_c(projection_res_path, "bias_corrected_ssps_regional_timeseries.csv"))
...
# Comparing Projected Climates to Observed Conditions



The following figure displays how SSP5 compares to observed actual SST measurements and the predicted future conditions through 2100. 

```{r}
#| label: ssp5-projection-graphic-coca
#| eval: true


# Reshape Joint Survey Timeseries -  why duplicated?
ssp5_wide <- ssp_both %>% 
  filter(scenario == "SSP5: 8.5") %>% 
  group_by(year, variable) %>% 
  summarise(across(c(`5th Percentile`, `95th Percentile`, `Ensemble Mean`), ~mean(.x, na.rm = T))) %>% 
  ungroup()

# Just sst
ssp5_wide_sst <- filter(ssp5_wide, variable == "Surface Temperature")
...
# What does it look like if I try and overlay some benchmarks... and did they change?
# need to subtract the baseline to get thresholds right
#find_crossing(, col_x = "5th Percentile", date_col = "year", threshold = 6)

# Original: used in coca reports
# label_df <- data.frame(
#       x = c(2045, 2062, 2081, 2099),
#       y = c(17),
#       label = str_c("+", c(1:4), deg_c))

# Or put labels on the line
label_df <- filter(
  ssp5_wide_sst, 
  year %in% c(2045, 2062, 2081, 2099)) %>% 
  mutate(
    label = case_when(
...
ussurvey_projected <- ggplot() +
  ...
  labs(fill = "Range of Climate Model Uncertainty\n(5th-95th Percentiles)",
       color = "SST:",
       x = NULL,
       y = str_c("Average Annual\nSea Surface Temperature"),
       subtitle = "Northeast U.S.",
       title = "Observed and Projected Sea Surface Temperatures") +
  theme(legend.box = "horizontal",
        legend.box.background = element_rect(color = "black"),
        legend.title = element_text(size = 16, family = "Avenir"),
        legend.text = element_text(size = 14, family = "Avenir"),
        legend.position = c(0.3, .81),
        axis.title.y = element_text(size = 18, family = "Avenir"),
        axis.text = element_text(size = 14, family = "Avenir"),
        plot.title = element_text(size = 18, family = "Avenir"),
        plot.subtitle = element_text(size = 16, family = "Avenir"),
...
```{r}
#| label: saving-ssp5-projection-graphic-coca

# # Save image for community reports
...
ggsave(
  filename = str_c(
    cs_path("mills", "Projects/COCA19/Community_Analyses/Report Graphics/Climate_Projection"),
    "NortheastUS_Scenario_vs_Obs_sst_big.png"),
  #filename = here::here("local_scrap/JointSurvey_Scenario_vs_Obs_sst_wide.png"),
  plot = ussurvey_projected,
  dpi = 300,
  bg = "white",
  width = 5.4,
  height = 2.5,
...
#        subtitle = "Northeast U.S.",
#        title = "Observed and Projected SST") +
#   theme(legend.box = "horizontal",
#         legend.box.background = element_rect(color = "black"),
#         legend.title = element_text(size = 16, family = "Avenir"),
#         legend.text = element_text(size = 14, family = "Avenir"),
#         legend.position = c(0.38, .88),
#         axis.title.y = element_text(size = 16, family = "Avenir"),
#         axis.text = element_text(size = 14, family = "Avenir"),
#         plot.title = element_text(size = 18, family = "Avenir"),
#         plot.subtitle = element_text(size = 16, family = "Avenir"),
#         strip.text = element_text(family = "Avenir"),
...
#   filename = str_c(
#     cs_path("mills", "Projects/COCA19/Community_Analyses/Report Graphics/Climate_Projection"),
#     "NortheastUS_Scenario_vs_Obs_sst_skinny.png"),
#   plot = ussurvey_projected,
#   dpi = 300,
#   bg = "white",
#   width = 4,
...
## Mid-Century Outlooks

For each climate scenario, what are the anticipated ranges in projected temp/salinity at the mid-century mark and at 2100?
...
# Baseline observed
obs_annual %>% filter(year %in% c(2010:2019)) %>% 
  summarise(across(!year, \(x) mean(x, na.rm = T)))
...
# Changes in Time Compared to Baseline Period
