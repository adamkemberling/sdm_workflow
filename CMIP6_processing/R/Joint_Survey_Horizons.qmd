...
# Clean up a little
js_ssp <- js_ssp %>% 
  select(scenario, ensemble_statistic, date, year, surf_temp, bot_temp, surf_sal, bot_sal)
...
modern_climate_annual <- js_obs %>% 
  ...
  summarise(
    across(.cols = c(surf_temp, bot_temp, surf_sal, bot_sal), mean)) %>% 
...
The following figure displays how SSP5 compares to observed data, and its predicted future trends. 
...
ssp5_wide <- js_ssp %>% 
  filter(scenario == "SSP5_85") %>% 
  select(-c(bot_temp, surf_sal, bot_sal)) %>% 
  pivot_wider(
    names_from = "ensemble_statistic", 
    values_from = "surf_temp") %>% 
  group_by(year) %>% 
  summarise(across(c(`5th Percentile`, `95th Percentile`, `Ensemble Mean`), mean)) %>% 
  ungroup() %>% 
  mutate(region = "hi")
...
jointsurvey_projected <- ggplot() +
  ...
  geom_line(
    data = obs_annual, 
    aes(year, surf_temp, color = "Observed SST"),
    linewidth = 1, key_glyph = "timeseries") +
  scale_fill_gmri(reverse = T) +
  scale_color_manual(values = c("black", gmri_cols("orange"))) +
  scale_x_continuous(limits = c(1982, 2100),
                     expand = expansion(add = c(2,1))) +
  scale_y_continuous(labels = scales::label_number(suffix = deg_c)) +
  theme_gmri() +
  guides(color = guide_legend(order = 1, nrow = 2, title.position = "top", title.hjust = 0.5), 
         fill = guide_legend(order = 2, title.position = "top", title.hjust = 0)) +
  labs(fill = "Climate Ensemble Uncertainty\n(5th-95th Percentile Range)", 
       color = "Average Annual Temperature:", 
       x = "", 
       y = str_c("Sea Surface Temperature"),
       title = "NMFS & DFO Shelf Survey Area",
       subtitle = "Observed and Projected Climate Under Ensemble Climate Scenario") +
  theme(legend.box = "horizontal",
        ...
        plot.subtitle = element_text(size = 12, family = "Avenir"),
        strip.text = element_text(family = "Avenir"))


#jointsurvey_projected
# ggsave(filename = here::here("local_scrap/JointSurvey_Scenario_vs_Obs_sst.png"), plot = jointsurvey_projected, dpi = 300, bg = "white")



# What does it look like if I try and overlay some benchmarks... and did they change?
label_df <- data.frame(
      x = c(2043, 2066, 2079, 2097),
      y = c(17),
      label = str_c("+", c(1:4), deg_c))
jointsurvey_projected +
  geom_point(
    data = label_df,
    aes(x, y), shape = "|", size = 2
  ) +
...
thresh_card <- function(
  ...
  t1 <- modern_comparison_annual %>% 
    filter(
      ...
      var == clim_var) %>% 
    find_crossing(., threshold = modern_thresh, col_x = "anom_ma", date_col = "year") 
  #return(t1)
